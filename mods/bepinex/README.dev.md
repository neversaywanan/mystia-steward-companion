...
夜间经营中，`经营中 / Service` 页会合并 `SpecialOrderRuntimeCapture` 捕获缓存与 `GuestsManager`、稀客队列、`OrderController`、HUD、服务面板和桌位控制器中的反射订单。捕获与反射互为补充，捕获缓存非空或诊断开关变化都不能改变最终订单集合；各来源中的已完成订单会在合并前过滤。快照通过 `orderRemovalVersion` 标识明确删除；伴随窗口遇到单个不同的空快照会继续显示 last-known-good，连续两个不同空快照才确认清空，而删除版本增长或经营场景变化会立即清空。页面同时读取活动稀客，用于显示当前稀客和 `GuestGroupController.GetFund`、`BaseFundCarry`、`MaxFundCarry` 等当前携带金钱信息。页面顶部只展示经营场景、扫描状态、推荐数据、厨具与置顶状态等通用信息，随后用 `稀客` / `普客` 页签分区展示各自功能。稀客点单后，工作台会按桌号列出稀客、料理词条和酒水词条，并复用稀客推荐算法计算候选料理、加料和酒水。普客订单读取到 `GameData.CoreLanguage.LanguageBase` 这类 IL2CPP 本地化对象时，必须过滤为无文本，不得把运行时类型名当作客人、料理或酒水名称展示。
