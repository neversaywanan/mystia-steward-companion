...
稀客推荐结果会按角色、点单词条、库存状态、厨具快照、排序配置、同基础料理展示数量和加料上限缓存。自动刷新没有检测到相关变化时，不会在每个刷新周期重复枚举加料组合；排序配置或同基础料理展示数量变化必须进入缓存签名，否则用户调整设置后会继续看到旧顺序或旧展示数量。
