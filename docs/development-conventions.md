...
- 稀客订单进入自动化后必须锁定本订单的料理、加料、酒水和可接受成品料理 ID 列表；后续轮询即使库存或排序导致推荐列表变化，也不能改用新的第一推荐，除非用户重置该订单或订单自然结束。自动化锁定排序后的统一推荐候选；若锁定料理标注为 `偏好备选`，状态中也必须保留该标识。只处理收藏配方开启时，锁定的料理和酒水都必须命中收藏。完成订单时若锁定目标料理不在送餐盘中，可以复用已在送餐盘中堆积超过阈值且属于该订单可接受成品列表的料理；不要复用未锁定的偏好备选或未知来源料理。
