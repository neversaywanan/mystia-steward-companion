...
- 场景切换后 Mod 不再做固定秒数等待；运行态读取由游戏 UI 生命周期事件放行。日间运行态以 `DaySceneSustainedPannel.OnPannelPostOpen` 为 ready 信号，并在 `OnPanelClosed` / `OnPrePanelDestroyed` 清理；夜间经营准备以 `PrepNightScene.UI.IzakayaConfigPannel.OnPanelOpen` / `GoToSpecific` 为 ready 信号，并在 `Cleanup_Generated` / `GotoWork` 清理。读取时仍要求对应面板处于激活状态，避免极端情况下 ready 状态残留。准备阶段只允许读取库存、解锁、流行 Tag 等基础玩家运行态，供 `修改`、`普客`、`稀客` 页面使用；任务、日间地图和稀客邀请仍只在日间读取。读取代码仍必须避开 IL2CPP `IEnumerator.Current` 这类加载阶段不稳定路径，优先用 Count/indexer、字段或静态快照读取，失败时返回状态提示并等待下一轮刷新，不得阻塞伴随窗口或影响游戏场景初始化。
...
- 任务列表、当前日间地图和稀客邀请读取必须使用独立的日间运行态判断：已加载存档、非非游戏页面、非夜间经营、非经营准备场景，并且已收到 `DaySceneSustainedPannel.OnPannelPostOpen` ready 信号。进入夜间经营准备时，只允许用 `WorkPrepScenePannelRoot` 下活跃的 `IzakayaConfigPannelNew` 和准备面板 ready 信号阻断日间读取；不要用泛化的同名面板或残留对象判断，否则日间场景可能被误挡，导致任务和邀请为空。
...
- 日间运行态读取只在 `DaySceneSustainedPannel.OnPannelPostOpen` 触发且面板仍激活后开始。空场景名、未知场景名或加载中的 Day 资源都不能兜底当作日间场景，否则主菜单或读档阶段仍可能触发 DataBase/Language 空引用。
